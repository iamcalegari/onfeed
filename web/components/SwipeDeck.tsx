...
// Inclinações determinísticas para dar aspecto de baralho desordenado
          ...
          <>
            {selected.map((h) => (
              <div key={h._id}
                ...
                <Link href={recipeHref(h._id, haveIds)}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative select-none"
        ...
        {/* Carta do meio (bg1) — com key estável: quando vira active, anima suavemente */}
        {bg1 && (
          <DeckCard
            key={bg1._id}
            hit={bg1}
        ...
        <DeckCard
          ...
          style={{
            transform: `translateX(${drag}px) rotate(${dragTilt}deg)`,
          ...
          {drag !== 0 && (
            <div
              className={`absolute left-4 top-16 rounded-lg border-2 px-3 py-1 text-lg font-extrabold ${
                drag > 0 ? "border-forest text-forest" : "border-terracota text-terracota"
              }`}
              style={{ opacity: hint }}
            >
              {drag > 0 ? "SIM" : "NÃO"}
      ...
      <div className="flex items-center justify-center gap-5">
        ...
        <Link href={recipeHref(current._id, haveIds)} aria-label="ver receita"
